...
class Kelompok extends CI_Controller {
    ...
    public function index(){
		...
		if(!empty($cek)){
			...
			$this->template->display('kelompok/home',$d);
		}else{
			?>
	...
	function load_data(){
		...
		if(!empty($cek)){
			...
			$sql = "";
			$sql .= "SELECT * FROM m_kelompok A0
					 WHERE ID IS NOT NULL";
			if ($param!='') {
				$sql .= " AND (A0.KELOMPOK LIKE '%$param%' OR INISIAL LIKE '%$param%')";
			}
			$sql .=" ORDER BY A0.ID";

			$d['data'] = $this->app_model->manualQuery($sql);
			$this->load->view('kelompok/load_data',$d);
		}else{
			?>
				<script type="text/javascript">
					alert('Maaf, Session anda telah habis. Silakan login ulang');
	...
	function simpan(){
		...
		$sql  = "";
		$sql .= "SELECT * FROM m_kelompok WHERE ID='$kode'";
		$stmt = $this->app_model->manualQuery($sql);
		if ($stmt->num_rows()<=0) {
			$sql = "";
			$sql .= "INSERT INTO m_kelompok(KELOMPOK,INISIAL) 
					 VALUES('$nama','$inisial')";
			...
			$sql = "";
			$sql .= "UPDATE m_kelompok SET KELOMPOK='$nama',INISIAL='$inisial'
					 WHERE ID='$kode'";
			$exec = $this->app_model->manualQuery($sql);
			if ($exec) {
				echo "edit_ok";
	...
	function hapus(){
		...
		$sql 	= "";
		$sql   .= "DELETE FROM m_kelompok WHERE ID='$kode'";
