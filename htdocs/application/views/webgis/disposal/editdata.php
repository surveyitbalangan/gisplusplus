<div class="content-wrapper">
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li class="active">Disposal <?php echo str_replace('/','-',substr($data_disposal->date, 0, 10)) ?></li>
        </ol>
    </section>
    <section class="content">
        <?php

        // var_dump($data_disposal);
        if ($data_disposal->company == 'LSA' || $data_disposal->company == 'lsa') {
            $comp = 'lsa';
        } else if ($data_disposal->company == 'SCM' || $data_disposal->company == 'scm') {
            $comp = 'scm';
        }
        ...
        <div class='col-md-12'>
            <?= form_open('disposal/update') ?>
        </div>
        <input id='objectid' type="hidden" name="objectid" value='<?= $data_disposal->objectid_1 ?>'>
        <div class="form-group">
            <label for="text">Tanggal</label>
            <input type="date" name="date" class="form-control" value="<?php echo str_replace('/','-',substr($data_disposal->date, 0, 10)) ?>" placeholder="Date">
            <!-- <input type="hidden" value="<?php echo $data_disposal->id_pit ?>" name="id_pit"> -->
        </div>

        <div class="form-group">
            <label for="text">Company</label>
            <select class='form-control' name="company" id="" value="<?php echo $data_disposal->company ?>">
        ...
        <script>
            ...
            leaflet = "<?= $data_disposal->st_astext ?>";
