<script type="text/javascript">
	$(window).on("load", function() {		
		...
		function simpan(){
			...
			$.ajax({
				type : 'POST',
				url  : '<?=base_url()?>parameter/simpan',
				data : {"nama":nama,"satuan":satuan,"jenis":jenis},
				cache: false,
				success : function(result){
					...
					}else if (result=='ilegal') {
						swal('Oops..','Data gagal disimpan\nData tersebut sudah ada!','warning');
					}
					load_data('');
		...
		function edit(){
			...
			$.ajax({
				type : 'POST',
				url  : '<?=base_url()?>parameter/edit',
				data : {"kode":kode,"nama":nama,"satuan":satuan,"jenis":jenis},
				cache: false,
				success : function(result){
					// alert(result);
					if (result=='edit_ok') {
		...
		function load_data(cari){
			$.ajax({
				type : 'POST',
				url  : '<?=base_url()?>parameter/load_data',
				data : {'cari':cari},
				cache: false,
				beforeSend: function() {
				  NProgress.start();
...
<div class="content-wrapper">
	<section class="content-header">
		...
		<ol class="breadcrumb">
			...
			<li class="active">Data Parameter</li>
		</ol>
	</section>
				...
				<div class="row">
					...
					<div class="col-md-3 col-sm-12">
						<div class="form-group has-feedback">							
							<input type="text" id="txtCari" class="form-control input-sm pull-right"  placeholder="Cari Nama Parameter..."/>			          	
			...
			<div class="modal-content">
				...
				<div class="modal-body">					
					<div class="form-group">
						<label class="control-label" for="txtNama">Parameter :</label>
						<input type="text" class="form-control" id="txtNama" size="5"/>
					</div>
                    <div class="form-group">
						<label class="control-label" for="cboSatuan">Satuan :</label>
						<select class="form-control select" id="cboSatuan">							
							<?php		                      			
								foreach ($satuan as $kode => $nama) {
                    ...
                    <div class="form-group">
						<label class="control-label" for="cboJenis">Jenis :</label>
						<select class="form-control select" id="cboJenis">							
							<option value="Pengupasan OB">Pengupasan OB</option>
                            <option value="Peledakan OB">Peledakan OB</option>
                            <option value="Penggunaan Bahan Peledak">Penggunaan Bahan Peledak</option>
                            <option value="Pengelolaan Air Tambang">Pengelolaan Air Tambang</option>
						</select>
					</div>           
				</div>
    ...
    <!-- Form edit data -->
	<div class="modal modal-default fade" id="form-edit" role="dialog" aria-hidden="true" style="display:none;">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				...
				<div class="modal-body">					
					<div class="form-group">
						<label class="control-label" for="txtNama_edit">Parameter :</label>
						<input type="text" class="form-control" id="txtNama_edit" size="5"/>
						<input type="hidden" class="form-control" id="txtKode" size="5"/>
					</div>
                    <div class="form-group">
						<label class="control-label" for="cboSatuan_edit">Satuan :</label>
						<select class="form-control select" id="cboSatuan_edit">							
							<?php		                      			
								foreach ($satuan as $kode => $nama) {
									echo "<option value='$kode'>$nama</option>";
								}
                    ...
                    <div class="form-group">
						<label class="control-label" for="cboJenis_edit">Jenis :</label>
						<select class="form-control select" id="cboJenis_edit">							
							<option value="Pengupasan OB">Pengupasan OB</option>
                            <option value="Peledakan OB">Peledakan OB</option>
                            <option value="Penggunaan Bahan Peledak">Penggunaan Bahan Peledak</option>
                            <option value="Pengelolaan Air Tambang">Pengelolaan Air Tambang</option>
				...
				<div class="modal-footer">					
					<button type="button" class="btn btn-info btn-flat" id="btnEdit"><i class="fa fa-sm fa-save"></i> Update</button>
					<button type="button" class="btn btn-warning btn-flat" data-dismiss="modal"><i class="fa fa-sm fa-mail-reply"></i> Batal</button>
				</div>
			</div>
		</div>
